...
# --- HELPER: GENE & STRUCTURE LOOKUP API ---
def fetch_gene_metadata(gene_symbol):
    """
    Fetches both the description AND the AlphaFold image URL via UniProt ID.
    """
    if not gene_symbol or gene_symbol == 'cluster':
        return "Select a gene", "Select a gene to see details.", None
    
    try:
        # Request 'uniprot' field in addition to name and summary
        url = f"https://mygene.info/v3/query?q=symbol:{gene_symbol}&species=human&fields=name,summary,uniprot"
        ...
        if 'hits' in data and len(data['hits']) > 0:
            ...
            # --- Try to extract UniProt ID for AlphaFold ---
            img_url = None
            if 'uniprot' in hit:
                # 'uniprot' can be a dictionary or list. We want 'Swiss-Prot' if possible.
                uni = hit['uniprot']
                uid = None
                if isinstance(uni, dict) and 'Swiss-Prot' in uni:
                    uid = uni['Swiss-Prot']
                    if isinstance(uid, list): uid = uid[0] # Handle rare list case
                elif isinstance(uni, str):
                    uid = uni
                
                # Construct AlphaFold URL if we found an ID
                if uid:
                    img_url = f"https://alphafold.ebi.ac.uk/files/AF-{uid}-F1-model_v4.png"

            return name, summary, img_url
        else:
            return "Unknown Gene", "No info found.", None
    except:
...
def create_app(data_input, mode='web', dataset_title="Human Lymph Node"):
    ...
    sidebar = dbc.Card(
        [
            ...
            html.Div(id='gene-info-container', children=[
                ...
                # --- NEW: AlphaFold Image Container ---
                html.Div(id='protein-structure-container', className="text-center my-3"),
    ...
    @app.callback(
        [Output('main-scatter', 'figure'),
         ...
         Output('protein-structure-container', 'children')], # NEW OUTPUT
        [Input('color-dropdown', 'value'),
         Input('view-toggle', 'value'),
         Input('show-image-check', 'value')]
    )
    def update_graph_and_info(color_col, view_mode, show_image):
        # A. Info & Structure Fetch
        img_element = None
        if color_col == 'cluster':
            g_name, g_sum = "Leiden Clustering", "Unsupervised clustering."
        else:
            g_name, g_sum, img_url = fetch_gene_metadata(color_col)
            # Create Image Element if URL found
            if img_url:
                img_element = html.Img(src=img_url, style={"width": "100%", "border-radius": "10px", "border": "1px solid #ddd"})
                g_sum = [html.Br(), g_sum] # Push summary down a bit
        ...
        # --- UPDATE: Set Dragmode to Pan ---
        fig.update_layout(
            title=dict(text=title, x=0.05, y=0.98),
            plot_bgcolor='white',
            margin=dict(l=20, r=20, t=50, b=20),
            dragmode='pan' # <--- HERE IS THE PAN FIX
        )
        ...
        return fig, g_name, g_sum, img_element
