...
def convert_anndata_to_df(adata):
    ...
    # 2. Metadata
    # We assume 'leiden' exists, if not, pick the first categorical column
    if 'leiden' in adata.obs:
        df['cluster'] = adata.obs['leiden'].astype(str)
    else:
        df['cluster'] = "1"

    # 3. Genes (We extract ALL genes for local mode, or a subset)
    # For performance in local CLI mode, we might only load HVGs or rely on on-the-fly extraction.
    # But for MVP, let's just use the dataframe structure for consistency.
    # NOTE: In a full CLI tool, we'd query adata dynamically. 
    # For this Sprint MVP, we will stick to the visualization structure.
    return df
